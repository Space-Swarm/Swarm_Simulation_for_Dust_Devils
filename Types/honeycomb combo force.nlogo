...
globals [G p FR D FMAX DeltaT VMAX FMAX_overall scale]

turtles-own [hood deltax deltay r F Fx Fy v vx vy t dvx dvy mass view square? counter Fmag]

to setup
   clear-all                                            ; Clear everything
   set p 2 set FMAX 1 set VMAX 2               ; Initialize force parameters
   set FMAX_overall 99
   set scale 0                                                     ; Create and initialize particles
...
to ap-particles                                         ; Run artificial physics
   ...
   set Fmag sqrt (Fx * Fx + Fy * Fy)
   if (Fmag > FMAX_overall) 
    [set Fy ((Fy) / Fmag) * FMAX_overall
    set Fx ((Fx) / Fmag) * FMAX_overall
    set F FMAX_overall]
...
@#$#@#$#@
I have modified the following to Spears' Model:

Implemented a overall Force Maximum in combination with the Individual Force Maximum already in place

Added a scaling coefficient for the force

Added a velocity maximum for the robots
